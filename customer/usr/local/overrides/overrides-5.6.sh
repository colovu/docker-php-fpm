...
sed -i -E 's/^pid = .*$/pid = \/var\/run\/php\/php5.6-fpm.pid/g' "${CONF_FILE}"
sed -i -E 's/^error_log = .*$/error_log = \/var\/log\/php\/php5.6-fpm.log/g' "${CONF_FILE}"
sed -i -E 's/^include=.*$/include=\/srv\/conf\/php\/5.6\/fpm\/pool.d\/\*.conf/g' "${CONF_FILE}"
